...
/**
 * What to call the offered version, in a badge or a one-line hint.
 *
 * `updateAvailable` covers three different things now that snapshots exist —
 * a new release, a new snapshot, and the step back to stable from a snapshot —
 * and calling all three "有新版本" would be wrong about the last one. The
 * sidebar, the settings tab and the dashboard all need the same phrasing, so
 * it lives here rather than as a ternary repeated three times.
 */
export function updateLabel(status: UpdateStatus | null): string | null {
  if (!status?.updateAvailable) return null
  if (status.downgrade) return '可回到正式版'
  return status.latestIsPrerelease ? '有新快照' : '有新版本'
}
