...
（`snapshot-86` 这种，标着 prerelease），想提前试新功能可以在更新页切过去，更新流程和正式版完全一样。
...
正式版（这是面板里唯一的一次「往回装」，见下）。生产环境请留在正式版通道：快照只保证过了 CI。

顺带说明一下**回退**：面板不支持装任意历史版本，正式版之间不能往回退，也点不到更早的快照 —— 更新页永远
只提供「最新的那一个」。唯一的例外就是上面这条从快照切回正式版的路。真要回到某个旧版本只能手动下载压缩包
替换二进制。
