...
第三块深色面是「启动方式」页的命令预览（`.launchcmd`）。它**借用 `--term-*` 而不另起一套**，因为它描述的正是服务器控制台里那个进程；它没有输入框，所以「敲错地方」的风险不存在。再要加深色面之前，先确认它同样是只读的。
...
| 内容宽度 | `--content-max` 880px（散文／表单）、`--content-max-wide` 1440px（瓦片） |

**宽度是「页」的属性，不是「卡片」的属性。** 一页要么整页是表单（880，`.stack--narrow`
或默认的 `Page`），要么整页是瓦片和表格（1440，`Page wide`）。一页上出现两种卡片宽度，
比宽卡片里留一段尾部空白更明显地读作「坏了」—— 这一条是试过才知道的：先给 `.panel--form`
单独封顶，结果表单和货架混排的页面立刻出现 1148 和 922 两种卡片。

间距没有令牌，是直接写的偶数 px。**沿用邻近区块的节奏**（页面级 `gap: 16px`、`.main` `gap: 12px`、卡片内 6/8/10/14px），不要凭空引入 13px、17px 这种值。

### 「左表单 + 右对照」

「启动方式」是目前唯一一种两栏页：左边是表单，右边是它产生的结果（命令预览 + 启动前检查）。

它**没有引入第四种测量**——用的就是实例 pane 里 `.stack` 已有的 1440 瓦片测量。两栏靠 `flex-wrap` 而不是断点：右栏低于某个宽度就读不下去了，而那个宽度与屏幕多大无关，所以该由容器决定什么时候换行。左栏 `flex: 3`、右栏 `flex: 1`，富余宽度归表单——均分的话右栏会比它所解释的那些字段还宽，读起来像这一页是关于命令的而不是关于设置的。

两栏都必须写 `min-width: 0`，命令块自己 `overflow-x: auto`：一条 Java 路径加二十个 Aikar 参数没有好看的折行方式，它是这一页唯一允许比页面宽的元素。

要再做一个这种页之前，先问它右边那半是不是**由左边算出来的结果**。如果只是「另一些内容」，那它是第二个 `Section`，不是第二栏。
...
「启动方式」页上四张卡片的标题各带一个 7px 色块，右栏命令预览的每一行行首也有一个——**同一个类**，所以卡片和它在命令行里对应的那一段不可能对不上，加一种来源时两处自动同步。

这是「只有异常才上色」的一个例外，也是它的边界：色块**承载信息**（这一段命令由哪张卡产生），不是用来让页面好看的。命令文字本身保持单色，只有 `panel`（面板注入的那组）压暗——它不是你填的，压暗正好读作「这个不归你改」，一个色相都不用新增。

给命令行本身上五种颜色的方案被否掉了：`--term-*` 只有两个色相，凑齐要新增 5 个令牌 × 8 个主题块，漏一处就是暗色下的空值。
