...
```
data/
├── panel.json        # 面板配置 + 已配对设备 + GitHub 令牌 (0600)
├── users.json        # 账号与角色，含密码哈希 (PBKDF2-SHA256, 0600)
...
都是普通 JSON，手改也行（改完重启面板）。`users.json` 里有一行格式不对，面板不会拒绝启动 ——
它会跳过那一行并在启动日志里说明，因为一个起不来的面板比一个少一个账号的面板更难修。实例目录也可以指向磁盘上任意已有的服务器，不一定要在
`servers/` 下面。
