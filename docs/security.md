...
**面板设置 → 账号与角色。** 建账号、改角色、勾实例授权都在这一页，需要「用户与角色」能力。
对应的接口是 `/api/users`、`/api/roles`、`/api/capabilities`。

角色编辑器里的能力列表是**面板给的**（`GET /api/capabilities`）而不是前端写死的，所以以后版本
新增一条能力，编辑器会自己多出一行。高危能力单独一组，上面写着它们实际意味着什么。

面板也按能力隐藏入口：侧栏、命令面板、「新建实例」按钮、实例页的分区、开关机按钮、控制台输入框。
**但隐藏不是屏障** —— 拦截在服务端按路由、按字段、按 WebSocket 消息做，前端少判一处只是毛刺，
不是漏洞。直接敲 URL 进一个没权限的页面，看到的是 403 或 404，不是内容。

### 目录限制

角色上可以填一组目录，这个角色的**文件管理器**就只能读写那些目录：
...
一行一个，相对实例目录，留空表示整个实例目录（这是默认，也是这个功能出现之前所有角色的状态）。

- **通往限制目录的上级目录可以浏览，但只显示那一条路。** 不然文件管理器根本走不到被授权的那个
  目录。上级目录里不属于这条路的东西**一个都不列** —— 目录列表不是用来告诉人有什么的。
- **范围外的一切都报「不存在」而不是「没权限」**，和实例授权同一个道理。
- 界面上：不在范围内的目录，上传和新建是灰的；不在范围内的条目，重命名和删除也是灰的。

**它只管文件管理器。** 编辑服务器配置、导入建筑、换核心、代理连线、启动检查各有各的能力，
走的是各自的路径，不受这里限制 —— 给了「编辑服务器配置」就是允许改 `server.properties`，
而那个文件在实例根目录，不会因为目录限制而变得不能改。

> [!WARNING]
> **只要这个角色同时持有任何一项能决定服务端跑什么代码的能力，目录限制对它就没有意义。**
> 装插件、改启动设置、换核心 —— 能执行代码就能绕开任何目录规则。角色编辑器在勾上这类能力时
> 会直接提示，并点名是哪几项。目录限制是给「只改自己那个插件的配置」这种职责划分用的，
> 不是用来约束一个本来就能执行代码的人。
