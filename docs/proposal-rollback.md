...
> **状态：第 1 期已实现。第 2 期（任选历史版本）还没做。**
>
> 更新页现在有「退回 `<上一版>`」，走本地 `<exe>.old`；任何降级之前都会备份面板自己的 JSON。
...
- `ReleaseByTag(ctx, tag)`：`GET /releases/tags/<tag>`。
...
- **`.old` 只有一份**。连退两版做不到，这是 `.old` 机制的固有限制，第 2 期的版本列表是它的补充。
