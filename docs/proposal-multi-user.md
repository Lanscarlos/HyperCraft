...
> **状态：第 1、2 步已实现。第 3 步（实例范围）和第 4 步（界面）待做。**
>
> 面板现在有账号、角色和能力鉴权。**还没有的是「这个人只管这几个服」** —— 任何账号的能力
> 都作用于全部实例。账号目前也只能通过 API 管理，界面是第 4 步。
...
- **`instances` 和 `paths` 这两个字段第 3 步才加。** 实现时刻意没有先把字段放进去：一个存在但
  不生效的授权字段，会让管理员以为自己限制了范围。JSON 加字段是向后兼容的，晚一步没有代价。
  加的时候 **`instances: null` 表示「全部」**，空数组表示「一个都没有」—— 用指针区分「没配过」
  和「明确清空」，和 `config.Panel.UpdateMirror` 是同一个理由。
...
3. **实例范围**：`instances` 字段 + `requireInstance`；`PUT /api/instances/{id}` 的危险字段做
   字段级检查；控制台 WebSocket 的双向检查；以及[实例不在路径里](#第-3-步要处理的实例不在路径里)
   那 5 条路由。
4. **前端**：`useCan()`、导航过滤、用户与角色管理页、实例授权编辑。
