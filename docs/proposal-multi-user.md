...
> **状态：第 1、2、3 步已实现。剩下界面（第 4 步）和目录白名单。**
>
> 面板现在有账号、角色、能力鉴权和实例授权。账号目前只能通过 API 管理，界面是第 4 步。
...
5. **目录白名单**（角色的 `paths`）。从第 3 步挪出来的：它是一个独立特性，而且在没有界面的情况下
   没法用 —— 一个只能用 curl 配的目录白名单，不会有人配对。
