...
发布产物是单文件二进制，前端已经嵌在里面，不需要 Go、Node 或者任何运行时依赖。部署就两件事：解压，交给 systemd。服务端要的 Java 也不用 `apt` —— 面板起来之后在「Java 运行时」页点一下就能装，所以下面这套在一台**什么都没装**的 Debian 上从头到尾能跑通。
...
3. 「启动设置」里选 Java 运行时、调内存 —— jar 下拉会自动列出目录里的文件。
   刚装好的机器上一个 Java 都没有，去侧栏的「Java 运行时」页一键装一个即可（1.20.5 起要
...
- 每台设备可以单独吊销：`GET /api/auth/devices` 看列表，`DELETE /api/auth/devices/{id}` 解除配对。
...
- **Java 运行时管理** —— 侧栏的「Java 运行时」页可以一键下载 Eclipse Temurin 的 JRE / JDK（任意大版本，LTS 有标注），
