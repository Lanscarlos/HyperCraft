...
登录成功和失败都会记日志，两个字段分别是 TCP 对端和面板判定的真实客户端：
...
没配 `trustedProxies` 时 `client` 就等于 `remote` 的地址部分。想知道加速器实际从哪个地址回源，
故意输错一次密码然后看 `remote=` 即可——这也是配防火墙白名单前确认清单对不对的办法。
...
- **插件管理（全局 + 实例两层）** —— 插件的**添加、来源、版本和更新统一在侧栏的「插件库」里管**，
  ...
  **全局（插件库）**：填一个 GitHub 仓库（`EssentialsX/Essentials`，或者直接把仓库地址粘进去），
  面板就从它的 Release 里拉 jar —— 和面板自己更新走的是同一条路，镜像源也跟着「设置 → 面板更新」
  里的那个走（默认 `https://ghfast.top/`，镜像失败自动回落 GitHub 直连）。一个 Release 里挂了
  ...
  **下载源可选**：ghfast.top、gh-proxy.com、github.moeyy.xyz、GitHub 直连，或者自己搭的代理前缀，
  在「设置 → 插件源」里选。默认自动，按顺序挨个试；指定了某一个，它不通也会回落到直连。镜像只搬
  ...
  **实例（「插件」标签）**：从库里挑一个插件和版本装进去 —— 拿到的是**自己的副本**，所以删掉库里的
  ...
  信任链落在 GitHub 的 TLS 和你填的那个仓库上 —— 这和 Java 运行时、服务端核心（都有上游发布的
  哈希可比对）不一样。
